...
const RecordingsPage: React.FC = () => {
  ...
  const totalRecordings = callsWithData.reduce(
    (count, call) => count + call.recordings.length,
    0
  );
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
        {callsWithData.map((callData, callIndex) =>
          callData.recordings.map((recording, recordingIndex) => {
            ...
            const recordingHasTranscription =
              callData.transcriptions &&
              callData.transcriptions.length > 0 &&
              callData.transcriptions.some(
