...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { message, uniqueId, previousMessages = [] } = requestBody;
    ...
    console.log(
      ...
      recordingMetadata.transcriptUrl
    );
    let transcript = "";
    try {
        ...
        if (response.ok) {
          ...
          try {
            ...
            if (Array.isArray(jsonArray)) {
              console.log(
                "Transcript parsed as JSON array, items:",
                jsonArray.length
              );
              transcript = jsonArray
                .map(
                  (segment) =>
                    `${segment.speaker_id || "Speaker"}: ${segment.text}`
                )
                .join("\n");
            }
          } catch (jsonError) {
    ...
    if (previousMessages.length > 0) {
      console.log(
        "Adding messages from current session:",
      ...
      for (const prevMsg of previousMessages) {
        ...
        if (
          !exists &&
          (prevMsg.role === "user" || prevMsg.role === "assistant")
        ) {
          formattedMessages.push({
            role: prevMsg.role,
